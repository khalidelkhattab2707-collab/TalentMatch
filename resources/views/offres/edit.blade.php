                    ...
                    <form method="POST" action="{{ route('offres.update', $offre) }}" class="space-y-6">
                        ...
                        <div>
                            ...
                            <div id="competences-wrapper" class="mt-1 space-y-2">
                                @foreach (old('competences_requises', $offre->competences_requises ?? ['']) as $index => $competence)
                        ...
                        <div>
                            ...
                            <x-text-input id="experience_minimum" name="experience_minimum" type="number" class="mt-1 block w-full" :value="old('experience_minimum', $offre->experience_minimum)" min="0" max="30" required />
                        ...
                        <div>
                            ...
                            <select id="statut" name="statut" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                <option value="active" {{ old('statut', $offre->statut->value) === 'active' ? 'selected' : '' }}>{{ __('Active') }}</option>
                                <option value="archivee" {{ old('statut', $offre->statut->value) === 'archivee' ? 'selected' : '' }}>{{ __('Archivée') }}</option>
                            </select>
