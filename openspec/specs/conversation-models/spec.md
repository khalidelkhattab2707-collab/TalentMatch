...
Provide application-specific Eloquent models (`App\Models\Conversation` and `App\Models\Message`) that extend the Laravel AI SDK's built-in models. These app models add convenience methods, custom relationships, and query scopes without modifying vendor code or changing the database schema.
